...
public interface VentaDao extends Dao<Venta, VentaSearchFilter, Integer> {
	...
	@Query("select new com.antoniojnavarro.naventory.model.dtos.GraficaVentaDto(date_format(v.fecha,'%m-%Y') , count(*)) from Venta v where v.usuario.email=?1 group by date_format(v.fecha,'%m-%Y')")
