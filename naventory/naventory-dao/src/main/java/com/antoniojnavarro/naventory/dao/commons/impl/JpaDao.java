...
import com.antoniojnavarro.naventory.model.commons.filters.annotations.InClauses;
import com.antoniojnavarro.naventory.model.commons.filters.annotations.ManualConditionWhere;
import com.antoniojnavarro.naventory.model.commons.filters.annotations.ManualConditionWheres;
...
public class JpaDao<T extends GenericEntity, F extends SearchFilter, ID extends Serializable>
	...
	private String buildCountDistinctSentence(String abbrEntity) {
		...
		for (int i = 0; i < idAttributeNames.size(); ++i) {
			result += abbrEntity + "." + idAttributeNames.get(i);
	...
	private void validateEntityFilter(EntityFilter entityFilter) {
		...
		Class<? extends GenericEntity> entity = entityFilter.entity();
		if (entity == null) {
			throw new IllegalArgumentException(
