package com.antoniojnavarro.naventory.app.config;
...
import com.antoniojnavarro.naventory.app.commons.ViewScope;
import com.antoniojnavarro.naventory.services.config.NaventoryServicesConfig;

@Configuration
@PropertySources({ @PropertySource(value = "file:${catalina.home}/conf/db.properties", ignoreResourceNotFound = false),
		@PropertySource(value = "file:${catalina.home}/conf/naventory-app.properties", ignoreResourceNotFound = false) })
@ComponentScan(basePackages = { "com.antoniojnavarro.naventory.app.*" })
@EnableAsync
@EnableAspectJAutoProxy
// Configuracion para Transaccionalidad a nivel @Service y la configuracion de
// los servicios Spring a utilizar
@ImportResource(value = { "WEB-INF/aop-config.xml" })
@Import({ NaventoryServicesConfig.class })
public class AppConfig {

	@Autowired
	private Environment env;

	@Resource(name = "packageModelJPANaventory")
	private List<String> packagesToScanJPANaventory;
	...
	public LocalContainerEntityManagerFactoryBean entityManagerFactory() {
		...
		emf.setPackagesToScan(packagesToScanJPANaventory.stream().toArray(String[]::new));
	...
	public static CustomScopeConfigurer customScopeConfigurer() {
		...
		customScopeConfigurer.addScope(PFScope.VIEW_SCOPED.toString(), ViewScope.getInstance());
